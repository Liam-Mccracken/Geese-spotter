...
function clearAll() {
    ...
    quoteContainer.innerHTML = ""; //inner html targets all in container so replacing empty means it empties it fully
    ...
    function showMeme() {
      ...
      clearAll(); // this will clear every instance 
      
      container.appendChild(newImg); //Adds the image to the document
    }
    ...
    container.appendChild(newp); // attaches the thing to the container
    ...
    function showQuote() {
     ...
     quote.textContent = randomQuote.quote;
      author.textContent = "- " + randomQuote.author; //concatinates the strings together 
      
      clearAll();
      
      document.querySelector('.quote-content').appendChild(quote); // alternative way to access the container just not assigning it to a variable
    ...
    function showRiddle() {
      ...
      const {question, answer} = randomRiddle; // this breaks down the elements of the object and allows you to use it   }
      const questionElem = document.createElement('p');
      questionElem.textContent = question; // adds the question to the element 
      
      const answerElem = document.createElement('p');
      answerElem.textContent = "the answer is " + answer; // adds the question to the element 
      answerElem.setAttribute('id','riddle-answer'); // this is creating a p tag with the id equal to the riddle answer, if its there and its hidden unhide it otherwise give error
      answerElem.hidden = true; // sets the hidden property to true 
    ...
    function revealAnswers() {
        const riddleContainer = document.querySelector('.riddle-content');
        const riddle = riddleContainer.querySelector('p'); // if any paragraph tag we know theres a riddle
        const answer = document.querySelector('#riddle-answer') // how to identify an ID
      
        if(riddle && answer.hidden){
        answer.hidden = false; // making sure the bit is turned off 
        }else if(riddle && answer){
          alert('An answer is already revealed');
        }else{
